...
GA4 的页面浏览由 React 在页面标题更新后手动发送，URL 会移除 hash、站内搜索词、
会员 ID/token 等查询参数，仅保留标准广告归因参数。为避免 SPA 路由被重复统计，请在
GA4「管理 → 数据流 → Web → 增强型衡量 → 网页浏览量 → 显示高级设置」中关闭
「根据浏览器历史记录事件进行网页更改」；页面加载项也由代码中的
`send_page_view: false` 关闭。
