...
function normalizeForm(input: unknown): WechatPublisherForm {
  const source = typeof input === "object" && input ? (input as Partial<WechatPublisherForm>) : {};
  const inlineList = Array.isArray(source.imagesInlineList) ? source.imagesInlineList : [];
  ...
  return {
    ...
    promptReferences: Array.isArray(source.promptReferences)
      ? source.promptReferences.filter((item): item is ReferenceType => item === "festivals" || item === "solarTerms")
      : [],
    imageCoverType: source.imageCoverType === "url" || source.imageCoverType === "base64" ? source.imageCoverType : "ai",
...
function getCompletionItems(form: WechatPublisherForm): CompletionItem[] {
  return [
    {
      label: "公众号配置",
      done: hasText(form.appId) && hasText(form.appSecret) && Boolean(form.articleType),
    },
    {
      label: "文章生成提示词",
      done: true,
    ...
    {
      label: "封面与内嵌图片",
      done: Boolean(form.imageCoverType) && hasText(form.imageCoverValue) && form.imagesInlineList.every((item) => Boolean(item.type) && hasText(item.value)),
    },
    {
      label: "文章元信息",
...
function getValidationErrors(form: WechatPublisherForm) {
  ...
  if (!hasText(form.imageCoverValue)) nextErrors.push("请填写或上传封面图生成值。");
  form.imagesInlineList.forEach((item, index) => {
    if (!item.type) nextErrors.push(`第 ${index + 1} 张内嵌文章图缺少图片类型。`);
    if (!hasText(item.value)) nextErrors.push(`第 ${index + 1} 张内嵌文章图缺少生成值。`);
  });
...
export function WechatPublisherPage() {
  const [form, setForm] = useState<WechatPublisherForm>(() => {
    try {
      return normalizeForm(JSON.parse(window.localStorage.getItem(storageKey) || "null"));
    } catch {
      return defaultForm;
    }
  });
  ...
  const imageCoverTypeLabel = imageTypeOptions.find((option) => option.value === form.imageCoverType)?.label ?? "AI 生成";

  function updateField<K extends keyof WechatPublisherForm>(key: K, value: WechatPublisherForm[K]) {
  ...
  async function handleInlineFile(index: number, event: ChangeEvent<HTMLInputElement>) {
    const file = event.target.files?.[0];
    if (!file) return;
  ...
  function updateInlineImage(index: number, patch: Partial<InlineImageItem>) {
    setForm((current) => ({
      ...current,
      imagesInlineList: current.imagesInlineList.map((item, itemIndex) => (itemIndex === index ? { ...item, ...patch } : item)),
    }));
  }
  ...
  function addInlineImage() {
    setForm((current) => {
      ...
      return {
        ...current,
        imagesInlineList: [...current.imagesInlineList, { type: "ai", value: "" }],
      };
    });
  }
  ...
  function removeInlineImage(index: number) {
    setForm((current) => ({
      ...current,
      imagesInlineList: current.imagesInlineList.filter((_, itemIndex) => itemIndex !== index),
    }));
  }
  ...
  async function handleGenerate(event: FormEvent<HTMLFormElement>) {
    ...
    const confirmed = window.confirm("生成公众号发布任务可能耗时较长，请确认是否开始？");
    if (!confirmed) return;

    setIsGenerating(true);
    setStatusText("正在整理提示词、图片素材和公众号参数，通常需要几十秒，请保持页面打开。");
    window.setTimeout(() => setStatusText("正在准备封面图与内嵌图片素材，Base64 文件会直接使用本地表单内容。"), 1600);
    window.setTimeout(() => setStatusText("正在模拟生成发布任务。真实发布接口接入后，会在这里显示任务结果。"), 3400);
    window.setTimeout(() => {
      setIsGenerating(false);
      setStatusText("已完成任务配置校验。当前版本已生成可提交的表单数据，等待后续接入真实发布服务。");
    }, 5200);
  }

  function handleReset() {
    const confirmed = window.confirm("重置会清空当前表单并覆盖本机自动保存内容，确认继续？");
    if (!confirmed) return;
    setForm(defaultForm);
    setErrors([]);
  ...
  async function handleCopyIp() {
    try {
      ...
      setStatusText(`无法自动复制，请手动将 ${apiWhitelistIp} 添加到 API IP 白名单。`);
    }
  }
      ...
      <section className="tool-form-page">
        ...
        <div className="tool-form-hero">
          ...
          <div className="json-actions" aria-label="JSON 配置操作">
            ...
            <input ref={importInputRef} className="sr-only" type="file" accept="application/json,.json" onChange={handleImport} />
          </div>
        </div>
          ...
          <div className="publisher-workspace">
            <div className="publisher-main">
              <section className="form-panel">
                ...
                <div className="form-grid two">
                  <label className="field">
                    <span>appId *</span>
                    <input value={form.appId} onChange={(event) => updateField("appId", event.target.value)} placeholder="请输入公众号 appId" required />
                  </label>
                  <label className="field">
                    <span>appSecret *</span>
                    <input
              ...
              <section className="form-panel">
                ...
                <label className="field">
                  <span>系统提示词</span>
                  <textarea value={form.promptSystem} onChange={(event) => updateField("promptSystem", event.target.value)} rows={4} placeholder="例如：你是一名专业公众号内容编辑..." />
                </label>
                <label className="field">
                  <span>主体内容提示词</span>
                  <textarea value={form.promptContent} onChange={(event) => updateField("promptContent", event.target.value)} rows={5} placeholder="输入文章主题、受众、语气、结构要求..." />
                </label>
                <fieldset className="choice-field">
                  <legend>参考信息</legend>
              ...
              <section className="form-panel">
                ...
                {form.imageCoverType === "base64" ? (
                  <label className="field">
                    ...
                    <small>{form.imageCoverValue ? "已保存 Base64 文件内容。" : "请选择图片文件，系统会转为 Base64 保存。"}</small>
                  </label>
                ) : (
                  <label className="field">
                    <span>封面图生成值 *</span>
                    <input
                      ...
                      onChange={(event) => updateField("imageCoverValue", event.target.value)}
                      placeholder={form.imageCoverType === "ai" ? "描述希望生成的封面图" : "https://example.com/cover.png"}
                      required
                    />
                  </label>
                )}
                  ...
                  {form.imagesInlineList.map((item, index) => (
                    <article className="inline-image-item" key={index}>
                      ...
                      {item.type === "base64" ? (
                        <label className="field">
                          <span>图片文件</span>
                          <input type="file" accept="image/*" onChange={(event) => handleInlineFile(index, event)} />
                          <small>{item.value ? "已保存 Base64 文件内容。" : "请选择图片文件。"}</small>
                        </label>
                      ) : (
                        <label className="field">
                          <span>图片生成值</span>
                          <input
                            value={item.value}
                            onChange={(event) => updateInlineImage(index, { value: event.target.value })}
                            placeholder={item.type === "ai" ? "描述这张内嵌图" : "https://example.com/inline.png"}
                          />
                        </label>
                      )}
                    </article>
              ...
              <section className="form-panel">
                ...
                <div className="form-grid two">
                  <label className="field">
                    <span>作者</span>
                    <input value={form.author} onChange={(event) => updateField("author", event.target.value)} placeholder="作者名称" />
                  </label>
                  <label className="field">
                    <span>原文链接</span>
                    <input value={form.sourceUrl} onChange={(event) => updateField("sourceUrl", event.target.value)} placeholder="https://example.com/source" />
                  </label>
                </div>
                <label className="field">
                  <span>摘要</span>
                  <textarea value={form.digest} onChange={(event) => updateField("digest", event.target.value)} rows={3} placeholder="用于公众号摘要展示的短文案" />
                </label>
                <fieldset className="choice-field">
                  <legend>评论配置 *</legend>
                  <label>
            ...
            <aside className="publisher-aside" aria-label="发布任务摘要">
              <section className="publish-summary-card">
                ...
                <div className="summary-checks">
                  {completionItems.map((item) => (
                    <div className={item.done ? "summary-check done" : "summary-check"} key={item.label}>
              ...
              <section className="publish-summary-card">
                ...
                <dl className="summary-list">
                  <div>
                    <dt>草稿类型</dt>
                    <dd>{form.articleType === "news" ? "news 图文消息" : "newspic 贴图/图片消息"}</dd>
                  </div>
                  <div>
                    <dt>封面方式</dt>
                  ...
                  <div>
                    <dt>评论</dt>
                    <dd>{form.comment === "open" ? "开放评论" : "仅粉丝评论"}</dd>
                  </div>
                </dl>
              </section>
